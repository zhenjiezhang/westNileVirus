...
# Plot the ROC curve for a simple model
library(randomForest)
dataFolder = "../input"
train = read.csv(file.path(dataFolder,"train.csv"))
train$year = as.POSIXlt(train$Date)$year + 1900
train$woy = as.POSIXlt(train$Date)$yday / 7
train$WnvPresent = as.factor(train$WnvPresent)
test = subset(train,year==2013)
train = subset(train,year!=2013)
m = randomForest(WnvPresent~Latitude+Longitude+woy+Species,data=train)
p = predict(m,newdata=test,type="prob")[,2]

png("ROCPlot.png",width=8,height=8,units="in",res=72)
plot.ROC(prediction(p,test$WnvPresent),0.25,"ROCplot.svg")
dev.off()
