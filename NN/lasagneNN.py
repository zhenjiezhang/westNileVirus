...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        case = [date.year, date.month*30+date.day, lat, longi]
        ...
        for days_ago in [0,1,2,3,5,8,12]:
            ...
            for obs in ["Tmax","Tmin","Tavg","DewPoint","WetBulb","PrecipTotal","Depart"]:
                ...
                sunRise=weather[day][0]['Sunrise']
                sunSet=weather[day][0]['Sunset']
                sunRise=int(sunRise)/100*60+int(sunRise)%100
                sunSet=int(sunSet)/100*60+int(sunSet)%100
                case.append(sunSet-sunRise)
